// AI Protocol NFTFactory: NFT Minting via Helper tests
...
const {
	eip712_mint,
	eip712_cancel,
} = new NFTFactoryEIP712("NFTFactoryV3");
...
contract("NFT Factory: minting an NFT via helper smart contract", function(accounts) {
	...
	describe("factory deployment", function() {
		it("fails if total mint hardcap is not set", async function() {
			await expectRevert(nft_factory_v3_deploy_restricted(a0, 0), "hardcap is not set");
		});
		describe("succeeds otherwise", function() {
			let factory;
			beforeEach(async function() {
				factory = await nft_factory_v3_deploy_restricted(a0, 13);
			});
			it("hardcap is set correctly", async function() {
				expect(await factory.totalMintHardcap()).to.be.bignumber.that.equals("13");
			});
		});
	});
	describe("when factory is deployed with default hardcap (10,000)", function() {
		let factory, nft;
		beforeEach(async function() {
			({factory, nft} = await nft_factory_v3_deploy(a0));
		});
		...
		describe("ACL", function() {
			...
			describe("when sender doesn't have ROLE_MINT_HARDCAP_MANAGER permission", function() {
				beforeEach(async function() {
					await factory.updateRole(operator, not(ROLE_MINT_HARDCAP_MANAGER), {from: a0});
				});
				it("update hardcap fails", async function() {
					await expectRevert(
						factory.updateTotalMintHardcap(10, {from: operator}),
						"access denied"
					);
				});
			});
			describe("when sender has ROLE_MINT_HARDCAP_MANAGER permission", function() {
				let receipt;
				beforeEach(async function() {
					await factory.updateRole(operator, ROLE_MINT_HARDCAP_MANAGER, {from: a0});
					receipt = await factory.updateTotalMintHardcap(10, {from: operator});
				});
				it("hardcap is updated", async function() {
					expect(await factory.totalMintHardcap()).to.be.bignumber.that.equals("10");
				});
				it('"TotalMintHardcapUpdated" event is emitted on the Factory', async function() {
					await expectEvent(receipt, "TotalMintHardcapUpdated", {
						authorizer: operator,
						oldLimit: new BN(10_000),
						newLimit: new BN(10),
					});
				});
			});
	...
	describe("when factory is deployed with lower hardcap (1)", function() {
		let factory, nft;
		beforeEach(async function() {
			({factory, nft} = await nft_factory_v3_deploy(a0, undefined, 1));
		});
